...
class UploadICPNew:
    ...
    def uploadBatch(self):
        ...
        for i in range(20):
            value = str(self.df.iloc[i,2]).lower()
            if "published:" in value:
                self.batchDate = str(self.df.iloc[i,3]).lower()

        cutoffIndex = 3

        self.batchId = self.batchDate
        ...
        sqlBatch = "SELECT icp_batch_id FROM icp_batches_1 WHERE date_run = ? AND " \
        ...
        # upload the batch
        sqlBatch = "INSERT INTO icp_batches_1 (date_run, project_id, operator, file_name, file_path) VALUES (?,?,?,?,?)"
        batchTuple = (
    ...
    def uploadReads(self):
        ...
        for index, row in self.df.iterrows():
            ...
            sqlSort = "INSERT INTO sort_chems_to_icp_batches_1 (sort_chem, icp_batch_id) VALUES(?,?)"
