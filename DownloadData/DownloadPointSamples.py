...
def downloadAllPointSamples(outputPath, ROOT, conn):
    ...
    # queries shows us which tables to pull data from
    # (the following should include all "grab sample" type data that we collect in person (ie on Synoptic)
    try:
        queries = [
        ...
        # There are lots of duplicate sortchem rows that contain different combinations of values,
        # this squashes all non-null values into one row
        rows = []
        seen_rows = []
        ...
        # has_hyphen includes all rows with dates we are able to parse.
        # The following lines clean the dates so that they can be converted into datetime objects
        ...
        final = final.rename(columns={
            'date': 'datetime', 'sort_chem': 'sortchem', 'temperature': 'temp_C_YSI', 'pressure': 'pressure_mmHg_YSI',
            'o2_percent': 'DO_perc_YSI', 'o2_mg': 'DO_mg_L_YSI', 'conductance': 'SPConductivity_uS_cm_YSI',
            'ph': 'pH_YSI', 'orp': 'ORP_mV_YSI', 'chlorophyl_ugl': 'chl_ug_L_YSI', 'chlorophyl_rfu': 'chl_RFU_YSI',
            'pc_ug': 'PC_ug_L_YSI', 'no3_from_srp': 'NO3_mg_L_SRP', 'nh4': 'NH4_mg_L_SRP', 'srp': 'SRP_mg_L_SRP',
            'tss': 'TSS_g_mL_SCAN', 'turbidity': 'turbidity_FTU_SCAN', 'no3': 'NO3_mg_L_SCAN', 'toc': 'TOC_mg_L_SCAN',
            'doc': 'DOC_mg_L_SCAN', 'aluminum': 'aluminum_mg_L_ICP', 'arsenic': 'arsenic_mg_L_ICP',
            'boron': 'boron_mg_L_ICP', 'barium': 'barium_mg_L_ICP', 'calcium': 'calcium_mg_L_ICP',
            'cadmium': 'cadmium_mg_L_ICP', 'cobalt': 'cobalt_mg_L_ICP', 'chromium': 'chromium_mg_L_ICP',
            'copper': 'copper_mg_L_ICP', 'iron': 'iron_mg_L_ICP', 'potassium': 'potassium_mg_L_ICP',
            'magnesium': 'magnesium_mg_L_ICP', 'manganese': 'manganese_mg_L_ICP', 'molybdenum': 'molybdenum_mg_L_ICP',
            'sodium': 'sodium_mg_L_ICP', 'nickel': 'nickel_mg_L_ICP', 'phosphorus': 'phosphorus_mg_L_ICP',
            'lead': 'lead_mg_L_ICP', 'sulfur': 'sulfur_mg_L_ICP', 'selenium': 'selenium_mg_L_ICP',
            'silicon': 'silicon_mg_L_ICP', 'strontium': 'strontium_mg_L_ICP', 'titanium': 'titanium_mg_L_ICP',
            'vanadium': 'vanadium_mg_L_ICP', 'zinc': 'zinc_mg_L_ICP', 'AVG(tic_mg_per_liter)': 'TIC_mg_L_elementar',
            'AVG(tc_mg_per_liter)': 'TC_mg_L_elementar','AVG(npoc_mg_per_liter)': 'NPOC_mg_L_elementar',
            'AVG(tnb_mg_per_liter)': 'TNb_mg_L_elementar','AVG(tic_area)': 'TIC_area_elementar',
            'AVG(tc_area)': 'TC_area_elementar', 'AVG(npoc_area)': 'NPOC_area_elementar',
            'AVG(tnb_area)': 'TNb_area_elementar', 'lithium_': 'lithium_ppb_IC', 'sodium_': 'sodium_ppb_IC',
            'ammonium_': 'ammonium_ppb_IC', 'potassium_': 'potassium_ppb_IC', 'magnesium_': 'magnesium_ppb_IC',
            'calcium_': 'calcium_ppb_IC', 'strontium_': 'strontium_ppb_IC', 'fluoride': 'fluoride_ppb_IC',
            'acetate': 'acetate_ppb_IC', 'formate': 'formate_ppb_IC', 'chloride': 'chloride_ppb_IC',
            'nitrite': 'nitrite_ppb_IC', 'bromide': 'bromide_ppb_IC', 'nitrate': 'nitrate_ppb_IC',
            'sulfate': 'sulfate_ppb_IC', 'phosphate': 'phosphate_ppb_IC', 'fi370': 'FI(FI370)_aqualog',
            'fi310': 'BIX(FI310)_aqualog', 'fi254': 'HIX(FI254)_aqualog', 'abs254': 'Abs254_aqualog',
            'slp274_295': 'slp274_295_aqualog', 'slp350_400': 'slp350_400_aqualog', 'sr': 'SR_aqualog',
            'ese3': 'ES_E3_aqualog', 'no3_from_lachat': 'NO3_mg_L_lachat', 'no4_ppm': 'PO4_mg_L_lachat',
        ...
        sections = {
            'Site and sampling info': [['sortchem', 'datetime', 'site_id', 'name_of_water_body', 'x', 'y', 'notes'],
                                       'red'],
            'Handheld sensor measurements': [
                ['temp_C_YSI', 'pressure_mmHg_YSI', 'DO_perc_YSI', 'DO_mg_L_YSI', 'SPConductivity_uS_cm_YSI', 'pH_YSI',
                 'ORP_mV_YSI', 'chl_ug_L_YSI', 'chl_RFU_YSI', 'PC_ug_L_YSI'], 'orange'],
            'Nutrients': [['NO3_mg_L_SRP', 'NH4_mg_L_SRP', 'SRP_mg_L_SRP'], 'yellow'],
            'Particulates': [['TSS_g_mL_SCAN'], 'green'],
            'Absorbance-device (S-CAN)': [['turbidity_FTU_SCAN', 'NO3_mg_L_SCAN', 'TOC_mg_L_SCAN', 'DOC_mg_L_SCAN'],
                                          'teal'],
            'ICP (mg/L)': [
                ['aluminum_mg_L_ICP', 'arsenic_mg_L_ICP', 'boron_mg_L_ICP', 'barium_mg_L_ICP', 'calcium_mg_L_ICP',
                 'cadmium_mg_L_ICP', 'cobalt_mg_L_ICP', 'chromium_mg_L_ICP', 'copper_mg_L_ICP', 'iron_mg_L_ICP',
                 'potassium_mg_L_ICP', 'magnesium_mg_L_ICP', 'manganese_mg_L_ICP', 'molybdenum_mg_L_ICP',
                 'sodium_mg_L_ICP', 'nickel_mg_L_ICP', 'phosphorus_mg_L_ICP', 'lead_mg_L_ICP', 'sulfur_mg_L_ICP',
                 'selenium_mg_L_ICP', 'silicon_mg_L_ICP', 'strontium_mg_L_ICP', 'titanium_mg_L_ICP',
                 'vanadium_mg_L_ICP', 'zinc_mg_L_ICP'], 'blue'],
            'Elemental analyzer (mg/L)': [
                ['TIC_mg_L_elementar', 'TC_mg_L_elementar', 'NPOC_mg_L_elementar', 'TNb_mg_L_elementar',
                 'TIC_area_elementar', 'TC_area_elementar', 'NPOC_area_elementar', 'TNb_area_elementar'], 'purple'],
            'IC data (ions in uM)': [
                ['lithium_ppb_IC', 'sodium_ppb_IC', 'ammonium_ppb_IC', 'potassium_ppb_IC', 'magnesium_ppb_IC',
                 'calcium_ppb_IC', 'strontium_ppb_IC', 'fluoride_ppb_IC', 'acetate_ppb_IC', 'formate_ppb_IC',
            ...
            'Fluorescence spectroscopy (EEMS)': [
                ['FI(FI370)_aqualog', 'BIX(FI310)_aqualog', 'HIX(FI254)_aqualog', 'Abs254_aqualog',
                 'slp274_295_aqualog', 'slp350_400_aqualog', 'SR_aqualog', 'ES_E3_aqualog'], 'brown'],
            "lachat": [["NO3_mg_L_lachat", "PO4_mg_L_lachat"], 'white'],
            'Q': [['discharge_m3_sec', 'date_collected', 'datetime_run', 'project_id_x', 'project_id_y'], 'black']}
...
def downloadUVUPointSamples(outputPath, ROOT, conn):
    ...
    try:
        ...
        df = pd.read_csv(os.path.join(ROOT, "Data/Fieldsheets/MasterFieldbook210831.csv"), encoding='mac-roman',
                         skiprows=1)
